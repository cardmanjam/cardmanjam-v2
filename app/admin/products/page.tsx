...
export default async function Products() {
  ...
  return <><p className="eyebrow">INVENTORY</p><h1>Products</h1><a className="btn" href="/admin/products/new">+ ADD PRODUCT</a><br/><br/>
    ...
    <tbody>{data?.map(p=><tr key={p.id}><td><a href={`/admin/products/${p.id}`}>{p.title}</a></td><td>${(p.price_cents/100).toFixed(2)}</td><td>{p.category}</td><td>{p.quantity}</td><td className="status">{p.status}</td></tr>)}</tbody></table></div>
  </>;
}
